...
def main():
    ...
    loss = PeriodicLoss(
        [nnet.nnmodel, 10.1, tch(bdry1, device=device), tch(bdry2, device=device)]
    )  # None #CustomLoss([nnet.nnmodel, 1.0])
    nnet.fit(
        xtrn, ytrn, val=[xval, yval], lrate=0.01, batch_size=10, nepochs=1000, loss=loss
    )
    print("=======================================")
    ...
    plot_fcn_2d_slice(
        NNWrap(nnet), domain, idim=0, jdim=1, nom=None, ngr=33, ax=axarr[1]
    )
